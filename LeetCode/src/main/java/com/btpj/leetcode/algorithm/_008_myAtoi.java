...
public class _008_myAtoi {
    public static int myAtoi(String s) {
        ...
        if (!(checkCharAvailable(charArr[0]))) return 0;

        for (char c : charArr) {
            if (checkCharAvailable(c)) {
                sb.append(c);
            } else {
                break;
            }
        ...
        try {
            ...
            return newStr.startsWith("-") ? Integer.MIN_VALUE : Integer.MAX_VALUE;
        }
    }

    private static boolean checkCharAvailable(char c) {
